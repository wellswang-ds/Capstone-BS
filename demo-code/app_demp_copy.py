...
articles = load_data('articles_filter.csv')
...
image_article_df = articles[["article_id","prod_name", "product_code", "product_group_name", "product_type_name", "section_name"]].merge(image_name_df, on=["article_id"], how="left")


def plot_image_samples(image_article_df, prod_name, cols=1, rows=-1):
    image_path = "/Users/wellswang/Documents/BrainStation/DS/Capstone/CapstoneProject-BS/notebooks/data/images/"
    _df = image_article_df.loc[image_article_df.prod_name==prod_name]
    article_ids = _df.article_id.values[0:cols*rows]
    # plt.figure(figsize=(2 + 3 * cols, 2 + 4 * rows))
    # for i in range(cols * rows):
    article_id = ("0" + str(article_ids[0]))[-10:]
    #     plt.subplot(rows, cols, i + 1)
    #     plt.axis('off')
    plt.figure()
    plt.title(f"{prod_name} {article_id[:3]}\n{article_id}.jpg")
...
image = plot_image_samples(image_article_df, top_selling, 1, 1)
# Display the image and DataFrame side by side
col1, col2 = st.columns([4,7])

col1.image(image, caption=top_selling, width=200)
...
col2.dataframe(data.reset_index(drop=True).set_index('product_id').T)
